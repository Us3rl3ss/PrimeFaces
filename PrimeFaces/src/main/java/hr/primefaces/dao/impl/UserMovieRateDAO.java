...
public class UserMovieRateDAO implements IUserMovieRateDAO, Serializable {
	...
	public List<UserMovieRate> getUserMovieRateByUserAndMovie(User user, Movie movie) {
		...
		String query = "from UserMovieRate " + "where user_id = :userId " + "and movie_id = :movieId";
	...
	public Double getAverageRateByMovie(Movie movie) {
		...
		String query = "select ROUND(avg(rate)) " + "from UserMovieRate " + "where movie_id = :movieId";

		Double result = (Double) getSessionFactory().getCurrentSession().createQuery(query).setParameter("movieId", movie.getId()).uniqueResult();
		return result;
	}
	...
	public List<UserMovieRate> getUserMovieRateByUser(User user) {
		...
		String query = "from UserMovieRate usr " + "join usr.movie " + "where user_id = :userId " + "order by usr.created desc";
