...
public class ProjectionDAO implements IProjectionDAO {
	...
	public Projection getProjectionById(int id) {
		List list = getSessionFactory().getCurrentSession()
				.createQuery("from Projection where id=?").setParameter(0, id)
				.list();
	...
	public List<Projection> getProjections() {
		List list = getSessionFactory().getCurrentSession()
				.createQuery("from Projection").list();
		return list;
	}
	...
	public List<Projection> getProjectionsByTheater(Theater theater) {
		...
		String query = "from Projection projection "
				+ "join projection.cinema cinema "
				+ "join cinema.theater theater "
				+ "join projection.movie movie "
				+ "where theater.id = ?";
		
		List list = getSessionFactory().getCurrentSession()
				.createQuery(query).setParameter(0, theater.getId()).list();
		return list;
	}
	...
	public List<Projection> getProjectionsForReservation(Theater theater) {
		...
		List list = getSessionFactory().getCurrentSession()
				.createQuery(query)
				.setParameter("theaterId", theater.getId())
				.list();
		return list;
	}
	...
	public List<Projection> getDistinctMovieProjections(Projection projection) {
		...
		String query = "from Projection where "
		...
		List list = getSessionFactory().getCurrentSession()
				.createQuery(query)
				.setParameter("theaterId", projection.getTheater().getId())
				.setParameter("movieId", projection.getMovie().getId())
				.list();
		return list;
	}
