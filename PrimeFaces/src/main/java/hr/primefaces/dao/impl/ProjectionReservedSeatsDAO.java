...
public class ProjectionReservedSeatsDAO implements IProjectionReservedSeatsDAO, Serializable {
	...
	public ProjectionReservedSeats getProjectionReservedSeatsById(int id) {
		List list = getSessionFactory().getCurrentSession().createQuery("from ProjectionReservedSeats where id=?").setParameter(0, id).list();
		return (ProjectionReservedSeats) list.get(0);
	}

	@Override
	public List<ProjectionReservedSeats> getProjectionReservedSeats() {
		List list = getSessionFactory().getCurrentSession().createQuery("from ProjectionReservedSeats").list();
		return list;
	}

	@Override
	public List<ProjectionReservedSeats> getProjectionReservedSeatsByName(String name) {
		List list = getSessionFactory().getCurrentSession().createQuery("from ProjectionReservedSeats where name like lower('%" + name.toLowerCase() + "%')")
				.list();
		return list;
	}
	...
	public List<ProjectionReservedSeats> getProjectionReservedSeatsByProjection(Projection projection) {
		...
		String query = "from ProjectionReservedSeats prs " + "where projection_id = ?";

		List list = getSessionFactory().getCurrentSession().createQuery(query).setParameter(0, projection.getId()).list();
		return list;
	}
	...
	public List<ProjectionReservedSeats> getProjectionReservedSeatsByProjectionAndUser(Projection projection, User user) {
		...
		String query = "from ProjectionReservedSeats prs " + "where projection_id = :projectionId and user_id = :userId";

		List list = getSessionFactory().getCurrentSession().createQuery(query).setParameter("projectionId", projection.getId())
